...
class TeamRepository(
    ...
    override fun getAll(): List<Team> = fetchTeamRows().mapNotNull(::rowToTeam)

    override fun getById(id: TeamId): Team? = getTeamRowById(id)?.let(::rowToTeam)

    override fun updateTeamName(
        id: TeamId,
        newName: TeamName,
    ): Team? {
        val updated =
    ...
    override fun updateTeamLogoName(
        id: TeamId,
    ...
    override fun updateEventId(
        ...
        val insertedId =
            dsl
                .update(TEAMS)
                .set(TEAMS.EVENT_ID, eventId?.value)
                .where(TEAMS.ID.eq(id.value))
                .returning(TEAMS.ID)
                .fetchOne()
                ?.get(TEAMS.ID)
    ...
    override fun insertPlayerToTeam(
        teamId: TeamId,
        playerId: PlayerId,
    ): TeamWithPlayers? {
        dsl
            .update(PLAYERS_TO_TEAM)
            .set(PLAYERS_TO_TEAM.TEAM_ID, teamId.value)
            .set(PLAYERS_TO_TEAM.PLAYER_ID, playerId.value)
        val updated =
            dsl
                .update(PLAYERS.playersToTeam)
    ...
    private fun getByTeamWithPlayersId(id: TeamId): TeamWithPlayers? = getTeamRowById(id)?.let(::rowToTeamWithPlayers)
