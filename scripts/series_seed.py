...
with open(f"{data_path}/series.json") as f:
    ...
    for d, matchups in data.items():
        ...
        if divisions is None:
            logger.warning(f"Failed to get event with teams {division}.")
            continue
        teams = divisions["teams"]
        for team, opps in matchups.items():
            team1 = dennys.find_team_id(team, teams)
            if team1 is None:
                logger.warning(f"Failed to find teamId for {team}")
                continue
            for opp in opps:
                team2 = dennys.find_team_id(opp, teams)
                if team2 is None:
                    logger.warning(f"Failed to find teamId for {opp}")
                    continue
                series = dennys.create_series(url, event_id, (team1, team2), 2)
                if series is None:
                    logger.warning(f"Failed to create series between {team1}, {team2}")
                else:
                    count = count + 1
    logger.info(f"Created {count} series.")
